...
EpochDeathBoardClick = {
	...
	if(isNil "_record_stxt") then {
		...
		_record_stxt = format["%1<t size='1' align='left'>Умер в %2:%3</t><br /><br />", _record_stxt, (_h call _format), (_m call _format)];
		
		if ((_record select 1) != 'unknown') then {
			_record_stxt = format["%1<t size='1' align='left'>Был убит %2</t><br /><br />", _record_stxt, (_record select 1)];
		};
		...
		if ((_record select 2) != 'unknown') then {
			...
			_record_stxt = format["%1<t size='1' align='left'>из %2<br /><img size='3' image='%3' /></t><br /><br />", _record_stxt, _name, _image];
		};
		
		if (format["%1", (_record select 3)] != 'unknown') then {
			_record_stxt = format["%1<t size='1' align='left'>с дистанции %2м</t><br /><br />", _record_stxt, (_record select 3)];
		};
