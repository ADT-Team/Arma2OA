...
/*
	...
	if (!_isAlive && !_isZombie && !_isAnimal && _hasETool && _isMan && _canDo) then {
        if (s_player_bury_human < 0) then {
            s_player_bury_human = player addAction [("<t color=""#FF6A00"">" + ("Похоронить") +"</t>"), "scripts\bury_human_adt.sqf",cursorTarget, 0, false, true, "", ""];
        }
    } else {
...
_isMedik = typeOf cursorTarget in ["pook_Doc_Bell47","Dr_Hladik_EP1","Dr_Annie_Baker_EP1"];
if((_isMedik && (player distance cursorTarget <= 3)) and _canDo) then {

        if (s_player_needhill1 < 0) then {
		s_player_needhill1 = player addAction [("<t color='#5882FA'>")+("Медицинская помощь")+("</t>"), "scripts\medical\healme.sqf","", -8, true, true, "", ""];
        };
    } else {
        player removeAction s_player_needhill1;
        s_player_needhill1 = -1;
        };
