...
if(_isTowing) then {
	...
	if(!(isNull _vehicle)) then {
		...
		if(!_hasToolbox) exitWith {
			cutText ["Нельзя отцепить без инструментов.", "PLAIN DOWN"];
		};
		...
		if (_finished) then {
			...
			cutText [format["%1 отцеплено от %2.", _vehicleNameText, _towTruckNameText], "PLAIN DOWN"];
	...
	cutText ["Техника не буксируется.", "PLAIN DOWN"];
};
