...
if !("PartOre" in magazines player) exitWith {cutText [format["Нужно 3 железной руды"], "PLAIN DOWN"]};
...
if (_finished) then {	
	...
	if ([["PartOre", 3]] call player_checkAndRemoveItems) then {
	...
	} else {cutText [format["Нужно 3 железной руды"], "PLAIN DOWN"];_objCraft = 0; DZE_ActionInProgress = false;};
	} else {
		DZE_ActionInProgress = false;
		r_interrupt = false;
		player switchMove "";
