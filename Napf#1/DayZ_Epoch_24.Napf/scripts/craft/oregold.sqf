...
if !("PartOreGold" in magazines player) exitWith {cutText [format["Нужно 2 золотой руды"], "PLAIN DOWN"]};
...
if (_finished) then {	
	...
	if ([["PartOreGold", 2]] call player_checkAndRemoveItems) then {
	...
	} else {cutText [format["Нужно 2 золотой руды"], "PLAIN DOWN"];_objCraft = 0; DZE_ActionInProgress = false;};
	} else {
		DZE_ActionInProgress = false;
		r_interrupt = false;
		player switchMove "";
