...
if (!isNull _menClose && _hasPatient && !r_drag_sqf && !r_action && !_inVehicle && !r_player_unconscious && _isClose) then {
	...
	if(_unconscious && _hasEpi) then {
		r_action = true;
		_action = _unit addAction [localize "str_actions_medical_05", "scripts\medical\epinephrine.sqf",[_unit], 0, true, true];
		r_player_actions set [count r_player_actions,_action];
	};
	//Allow player to give Morphine
	if((_legsBroke || _armsBroke) && _hasMorphine) then {
		r_action = true;
		_action = _unit addAction [localize "str_actions_medical_06", "scripts\medical\morphine.sqf",[_unit], 0, true, true, "", ""];
		r_player_actions set [count r_player_actions,_action];
	};
	//Allow player to give Painkillers
	if(_inPain && _hasPainkillers) then {
		r_action = true;
		_action = _unit addAction [localize "str_actions_medical_07", "scripts\medical\painkiller.sqf",[_unit], 0, true, true, "", ""];
		r_player_actions set [count r_player_actions,_action];
	};
	//Allow player to transfuse blood
	if(_lowBlood && _hasBlood) then {
		r_action = true;
		_action = _unit addAction [localize "str_actions_medical_08", "scripts\medical\transfusion.sqf",[_unit], 0, true, true, "", ""];
		r_player_actions set [count r_player_actions,_action];
	};
	...
	if ((_unit isKindOf "AllVehicles") && !(_unit isKindOf "Man") && !_isDisallowRefuel) then {
		...
		if ((fuel _unit > 0) && (_hasJerryE || _hasBarrelE)) then {
			r_action = true;
			_action = _unit addAction [format["Слить топливо с %1",_typeVeh], "\z\addons\dayz_code\actions\siphonFuel.sqf",[], 0, true, true, "", ""];
			r_player_actions set [count r_player_actions,_action];
		};
		...
		if ((isPlayer _unit) && !(_charID in _friendlies)) then {
			r_action = true;
			_action = _unit addAction ["Добавить в друзья", "\z\addons\dayz_code\actions\player_tagFriendly.sqf", [], 0, false, true, "", ""];
			r_player_actions set [count r_player_actions,_action];
		};
