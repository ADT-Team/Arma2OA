...
_bloodAmount = 5000; // Total amount of blood to give back to player on sleep (12000 is the max life for a player)

_sleepCooldown = 180; // This is how long in seconds before you can sleep again
...
if(_sleepTime < _sleepCooldown) exitWith { // If cooldown is not done then exit script
	cutText [format["Я не устал! Посплю позже...",(_sleepTime - _sleepCooldown)], "PLAIN DOWN"]; //display text at bottom center of screen when players cooldown is not done
};

if (dayz_combat == 1) then { // Check if in combat (dunno why you would want to sleep in combat but the check is here anyway)
    cutText [format["Нельзя спать в бою! ;)"], "PLAIN DOWN"]; //display text at bottom center of screen when in combat
} else {
	...
	for "_i" from 0 to _totalSleepTime do { // For loop for counting down sleep timers and changing animations
		...
		if (_timeLeft > 0) then {
			cutText [format["Я сплю! Проснусь секунд через ...%1",_timeLeft], "BLACK FADED"]; // Display sleep countdown while sleeping
		} else {cutText [format["Эх... как не охото вставать... Еще чуть чуть..."], "BLACK FADED"];}; // This is a fix for when timer reaches zero... displays text
	...
	if (_timeLeft == 0 || _timeLeft < 0) then { // If sleep countdown is done then.....
		...
		cutText [format["Выспался! Я полон сил и здоровья! Только покушать бы чего-нибудь..."], "PLAIN DOWN"]; // Text displayed on wakeup!
